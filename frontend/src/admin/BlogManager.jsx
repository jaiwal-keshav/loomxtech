    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="glass-card mt-8 grid place-items-center p-12 text-center">
          <Icon name="code" className="h-10 w-10 text-slate-600" />
          <p className="mt-3 text-slate-400">No posts yet.</p>
          ...
          {posts.map((p) => (
            <div key={p.id} className="glass-card flex items-center justify-between gap-4 p-4">
              <div className="min-w-0">
                ...
                <div className="mt-1 truncate text-xs text-slate-500">/{p.slug} · {p.published ? 'Published' : 'Draft'}</div>
              </div>
              <div className="flex shrink-0 items-center gap-2">
                <Link to={`/admin/blog/${p.id}`} className="rounded-lg border border-white/10 px-3 py-1.5 text-xs text-slate-300 hover:border-neon-cyan/50 hover:text-white">
                  Edit
                </Link>
