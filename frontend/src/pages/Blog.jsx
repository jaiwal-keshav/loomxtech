    ...
    <>
      ...
      <section className="container-x py-12">
        {loading ? (
          ...
          <p className="text-center text-slate-400">Couldn't load posts right now. Please try again later.</p>
        ) : posts.length === 0 ? (
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {posts.map((p, i) => (
              <Reveal key={p.id} delay={(i % 3) * 0.06}>
                <Link
                  to={`/blog/${p.slug}`}
                  className="group flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] transition hover:-translate-y-1 hover:border-neon-cyan/40 hover:shadow-glow"
                >
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    ...
                    <h3 className="mt-2 font-display text-lg font-semibold text-white group-hover:text-neon-cyan">
                      {p.title}
                    </h3>
                    <p className="mt-2 flex-1 text-sm leading-relaxed text-slate-400 line-clamp-3">{p.excerpt}</p>
